...
## Initial Request — 2026-07-11T13:33:25+07:00
...
### UI/UX Polish
- [ ] Giao diện toàn hệ thống đồng nhất, không bị vỡ layout ở mọi kích thước màn hình (Mobile, Tablet, Desktop).
- [ ] Mọi thay đổi về phong cách CSS và màu sắc vẫn giữ được các nét thiết kế gốc mà người dùng đã quen thuộc.
- [ ] Toàn bộ các tương tác nút bấm, form nhập liệu hoạt động mượt mà và không gây lỗi giao diện.

### Charts & Analytics
- [ ] Một biểu đồ mới hiển thị xu hướng chi tiêu theo ngày (`dailyTrendChart`) được dựng thành công trong màn hình Analytics.
- [ ] Biểu đồ cập nhật chính xác dữ liệu từ `caltdhy_db.json` thông qua các API load/save có sẵn.
- [ ] Trang spending chạy ổn định không có lỗi uncaught JavaScript trong console.

## Verification Plan

### Automated Tests
- Chạy file kiểm thử tự động cấu trúc HTML và kiểm tra logic kết nối biểu đồ bằng lệnh:
  `node verify_ui_charts.js`

### Manual Verification
- Người dùng mở giao diện, chuyển sang tab Analytics để kiểm tra sự xuất hiện của biểu đồ xu hướng hàng ngày và thực hiện thêm giao dịch để xem biểu đồ tự động cập nhật dữ liệu.
