...
Production mặc định chỉ cho đăng ký bằng lời mời một lần, gắn với email và hết hạn; script quản trị tạo URL mời được mô tả trong [hướng dẫn PWA](PWA-DEPLOYMENT.md). Đăng ký tạo tài khoản chưa xác minh email. Không coi cờ đăng nhập là bằng chứng email đã xác minh. Luồng gửi lại email xác minh và đặt lại mật khẩu cần `CLIENT_URL` hợp lệ cùng Brevo API qua HTTPS hoặc Gmail SMTP ở môi trường cho phép SMTP; khi chưa cấu hình, API trả 503. Render Free không cho kết nối các cổng SMTP thông dụng, nên chọn Brevo tại đây. Phản hồi chung khi đã cấu hình không bảo đảm thư đã gửi. Không ghi token/link reset hoặc API key vào log. Gửi thư production cần được kiểm tra riêng bằng tài khoản thử nghiệm được phép.
